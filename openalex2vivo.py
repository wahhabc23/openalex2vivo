...
from openalex2vivo_app.vivo_uri import HashIdentifierStrategy
from openalex2vivo_app.vivo_namespace import VIVO, FOAF, VCARD
from openalex2vivo_app.affiliations import AffiliationsCrosswalk
from openalex2vivo_app.bio import BioCrosswalk
from openalex2vivo_app.works import WorksCrosswalk
...
class PersonCrosswalk():
    ...
    def crosswalk(self, orcid_id, person_uri, person_class=None):
        ...
        openalex_profile = fetch_openalex_profile(orcid_id)
        resolved_orcid_id = openalex_profile.get("id")
        ...
        # OpenAlex ID
        if resolved_orcid_id:
            PersonCrosswalk._add_orcid_id(person_uri, resolved_orcid_id, graph)

        self.bio_crosswalker.crosswalk(openalex_profile, person_uri, graph, person_class=person_clazz)
        self.works_crosswalker.crosswalk(openalex_profile, person_uri, graph)
        self.affiliations_crosswalker.crosswalk(openalex_profile, person_uri, graph)

        return graph, openalex_profile, person_uri

    @staticmethod
    def _add_orcid_id(person_uri, orcid_id, graph):
        orcid_id_uriref = URIRef(orcid_id)
        graph.add((person_uri, VIVO.openalexId, orcid_id_uriref))
        graph.add((orcid_id_uriref, RDF.type, OWL.Thing))


def fetch_openalex_profile(input_id):
    """
    input_id can be an ORCID (e.g., 0000-0002-1825-0097) or an OpenAlex ID (e.g., A5100335963).
    If it's an ORCID, query the API using the ORCID filter.
    """
    input_id = input_id.strip()
    if input_id.startswith("http"):
        # Assume it's a full URL
        url = input_id
    elif "0000-" in input_id or (len(input_id) == 19 and input_id.replace('-', '').isdigit() or "X" in input_id):
        # looks like an ORCID
        # strip potentially prefixed https://orcid.org/
        clean_orcid = input_id.replace('https://orcid.org/', '').replace('http://orcid.org/', '')
        url = 'https://api.openalex.org/authors?filter=orcid:%s' % clean_orcid
    else:
        # Assume it's an OpenAlex Author ID like A5100335963
        url = 'https://api.openalex.org/authors/%s' % input_id
    ...
    if r.status_code == 200:
        ...
        if "results" in data:
            if len(data["results"]) == 0:
                raise Exception("No OpenAlex profiles found for %s" % input_id)
            return data["results"][0]
        else:
            return data
    else:
        raise Exception("Request to fetch OpenAlex profile for %s returned %s" % (input_id, r.status_code))
