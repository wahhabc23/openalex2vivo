...
import openalex2vivo_app.utility as utility
...
def crosswalk_form(rdf=None, openalex_profile=None):
    return render_template("crosswalk_form.html",
                           ...
                           openalex_profile=json.dumps(openalex_profile, indent=3) if openalex_profile else None)
...
def crosswalk():
    ...
    if "output" in request.form and request.form["output"] == "vivo":
        ...
        if endpoint.endswith("api/sparqlUpdate"):
            vivo_profile_url = "%s/individual?%s" % (endpoint[:-17], urllib.parse.urlencode({"uri": per_uri}))
        ...
        if "output_html" in request.form or "output_profile" in request.form:
            return crosswalk_form(rdf=rdf if "output_html" in request.form else None,
                                  openalex_profile=p if "output_profile" in request.form else None)
        else:
