...
import tempfile
import shutil
from openalex2vivo_loader import Store, load_single
import os
import tests
import time
import datetime
...
class TestStore(tests.TestCase):
    def setUp(self):
        ...
        self.db_filepath = os.path.join(self.data_path, "openalex2vivo.db")
    ...
    def test_add_item(self):
        with Store(self.data_path) as store:
            # Insert
            store.add("0000-0003-1527-0030")
            (openalex_id, active, last_update, person_uri, person_id, person_class, confirmed) = \
    ...
    def test_touch(self):
        with Store(self.data_path) as store:
            ...
            (openalex_id, active, new_last_update, person_uri, person_id, person_class, confirmed) = \
                store["0000-0003-1527-0030"]
    ...
    def test_iter(self):
        with Store(self.data_path) as store:
            store.add("0000-0003-1527-0030")
            store.add("0000-0003-1527-0031")
            for openalex_id, active, new_last_update, person_uri, person_id, person_class, confirmed in store:
                self.assertTrue(openalex_id in ("0000-0003-1527-0030", "0000-0003-1527-0031"))
...
class TestLoad(tests.TestCase):
    ...
    @patch("openalex2vivo_loader.sparql_insert")
    @patch("openalex2vivo_loader.sparql_delete")
    def test_load_single(self, mock_sparql_delete, mock_sparql_insert):
        ...
        # r = requests.get('https://pub.openalex.org/v2.0/%s' % openalex,
