...
from openalex2vivo_app.vivo_namespace import ns_manager
from openalex2vivo_app.utility import sparql_insert, sparql_delete
...
class Store:
    def __init__(self, data_path):
        self.db_filepath = os.path.join(data_path, "openalex2vivo.db")
    ...
    def __contains__(self, orcid_id):
        """
        Returns True if there is a record for the openalex id and it is active.
        """
    ...
    def contains(self, orcid_id, active=None):
        """
        Returns True if there is a record for the openalex id.
        """
    ...
    def __getitem__(self, orcid_id):
        """
        Returns orcid_id, active, last_update, person_uri, person_id, person_class, confirmed for openalex id.
        """
        c = self._conn.cursor()
        c.execute("""
    ...
    def __delitem__(self, orcid_id):
        """
        Marks an openalex id as inactive.
        """
        c = self._conn.cursor()
    ...
    def add(self, orcid_id, person_uri=None, person_id=None, person_class=None, confirmed=False):
        """
        Adds openalex id or updates existing openalex id and marks as active.
        """
        c = self._conn.cursor()
    ...
    def get_least_recent(self, limit=None, before_datetime=None):
        """
        Returns least recently updated active openalex ids as list of
        orcid_id, person_uri, person_id, person_class, confirmed.
        """
        c = self._conn.cursor()
        sql = """
    ...
    def touch(self, orcid_id):
        """
        Set last update for openalex id.
        """
        c = self._conn.cursor()
...
def load(data_path, endpoint, username, password, limit=None, before_datetime=None, namespace=None, skip_person=False):
    ...
    failed_orcid_ids = []
    with Store(data_path) as store:
        # Get the openalex ids to update
...
if __name__ == "__main__":
    ...
    add_parser = subparsers.add_parser("add", help="Adds or updates openalex id record. If inactive, marks active.",
    ...
    parser.add_argument("--confirmed", action="store_true", help="Mark the openalex id as confirmed.")

    delete_parser = subparsers.add_parser("delete", help="Marks an openalex id record as inactive so that it will not be "
                                                         "loaded.",
                                          parents=[orcid_id_parent_parser, data_path_parent_parser])

    delete_all_parser = subparsers.add_parser("delete-all", help="Marks all openalex id records as inactive.",
                                              parents=[data_path_parent_parser])

    load_parser = subparsers.add_parser("load", help="Fetches openalex profiles, crosswalks to VIVO-ISF, loads to VIVO "
                                                     "instance, and updates openalex id record. If loading multiple "
                                                     "openalex ids, loads in least recent order.",
                                        parents=[data_path_parent_parser])
    ...
    load_parser.add_argument("--orcid_id", help="OpenAlex id of person to load.")
    load_parser.add_argument("--limit", type=int, help="Maximimum number of openalex ids to load.")
    load_parser.add_argument("--before", help="OpenAlex ids that were loaded before this date or never loaded. Format is "
